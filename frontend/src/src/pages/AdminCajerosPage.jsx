...
export function AdminCajerosPage() {
    ...
    async function load() {
        setLoading(true);
        try {
            const data = await apiFetch('/api/admin/cajeros');
            setCajeros(Array.isArray(data?.data) ? data.data : []);
        } catch (err) {
            void adminAlertError(err, 'No se pudieron cargar los cajeros');
        } finally {
            setLoading(false);
        }
    }
    ...
    async function saveDraft(e) {
        ...
        try {
            if (draft.idUsuario) {
                await apiFetch(`/api/admin/cajeros/${draft.idUsuario}`, {
                    method: 'PUT',
                    body: JSON.stringify(payload),
                });
            } else {
                await apiFetch('/api/admin/cajeros', {
                    method: 'POST',
                    body: JSON.stringify(payload),
                });
            }
            ...
            void adminAlertError(err, 'No se pudo guardar el cajero');
        } finally {
            setSaving(false);
        }
    }
    ...
    async function toggleActivo(m) {
        try {
            await apiFetch(`/api/admin/cajeros/${m.idUsuario}/activo`, {
                method: 'PATCH',
    ...
    if (loading) {
        return (
            <AdminLayout title="Cajeros">
                <div className="flex items-center justify-center text-stone-600 dark:text-stone-400 text-lg py-20">Cargando cajeros…</div>
            </AdminLayout>
        );
    }
    ...
    return (
        <AdminLayout title="Cajeros">
            <div className="flex items-start justify-between gap-6 flex-wrap">
                <div>
                    <div className="text-3xl font-semibold tracking-tight">Cajeros</div>
                    <div className="mt-2 text-stone-600 dark:text-stone-400">Crea, edita o deshabilita cajeros (sin eliminarlos).</div>
                </div>
                <button
                    onClick={openCreate}
                    className="bg-blue-700 hover:bg-blue-600 text-stone-50 font-semibold rounded-lg px-6 py-3 transition-colors focus-visible:ring-2 focus-visible:ring-amber-500"
                >
                    Crear cajero
                </button>
            </div>
                        ...
                        <tbody className="divide-y divide-stone-200 dark:divide-stone-800">
                            {cajeros.map((m) => (
                                <tr key={m.idUsuario} className="hover:bg-stone-100/70 dark:hover:bg-stone-800/35">
                                    ...
                                    <td className="px-4 py-3">
                                        {m.activo ? (
                                            <span className="inline-flex items-center rounded-full border border-blue-500/30 bg-blue-600/15 px-2 py-0.5 text-xs text-blue-900 dark:text-blue-200">
                                                Activo
                                            </span>
                                        ) : (
                            ...
                            <div className="flex items-start justify-between gap-6">
                                <div>
                                    <div className="text-lg font-semibold">
                                        {draft.idUsuario ? 'Editar cajero' : 'Crear cajero'}
                                    </div>
                            ...
                            <form className="mt-6 space-y-4" onSubmit={saveDraft}>
                                ...
                                <div className="pt-2 flex items-center justify-end gap-3">
                                    ...
                                    <button
                                        ...
                                        disabled={saving}
                                        className={classNames(
                                            'bg-blue-700 hover:bg-blue-600 text-stone-50 font-semibold rounded-lg px-6 py-3 transition-colors focus-visible:ring-2 focus-visible:ring-amber-500',
